...
import { ChevronLeft, ClipboardList, User, Calendar, Info, Search, AlertTriangle } from 'lucide-react';
import { getAllAuditLogs, translateAction } from '../supabaseService';
                ...
                <div className="space-y-3">
                    {filteredLogs.length === 0 ? (
                        ...
                        filteredLogs.map((log) => (
                            <div key={log.id} className="bg-surface-light dark:bg-surface-dark p-4 rounded-xl border border-border-light dark:border-border-dark shadow-sm hover:border-primary/50 transition-colors">
                                <div className="flex items-start justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-1 text-[10px] font-bold text-primary px-2 py-1 bg-primary/5 rounded">
                                        ID Mandado: {log.warrant_id ? log.warrant_id.substring(0, 8) : 'N/A'}
                                    </div>
                                </div>
